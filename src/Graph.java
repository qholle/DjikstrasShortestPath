...
public class Graph<T> implements GraphADT<T> {
  ...
  public String toString() {
    String allNodes = vertices.values().toString();
    return allNodes;
  }
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      this.dataSequence.add(copyPath.start.data);
      this.distance = 0;
      for (int i = 1; i < copyPath.dataSequence.size(); i++) {
        this.dataSequence.add(copyPath.dataSequence.get(i));
        this.distance = this.distance
            + getWeight(copyPath.dataSequence.get(i - 1), copyPath.dataSequence.get(i));
      }
      this.dataSequence.add(extendBy.target.data);
      this.distance =
          this.distance + getWeight(copyPath.dataSequence.get(copyPath.dataSequence.size() - 1),
              extendBy.target.data);
    ...
    public String toString() {
      ...
      for (int i = 0; i < dataSequence.size(); i++) {
        pathString = pathString + dataSequence.get(i).toString() + "-> ";
      }
      return pathString;
    }
  }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // add all vertices in graph to a priority queue
    LinkedList<Vertex> unvisitedNodes = new LinkedList<Vertex>();
    LinkedList<Vertex> visited = new LinkedList<Vertex>();
    PriorityQueue<Path> foundPaths = new PriorityQueue<Path>();

    unvisitedNodes.addAll(vertices.values());

    // throw NoSuchElementException if the graph doesn't contain the start or end nodes
    if (!this.containsVertex(start) || !this.containsVertex(end)) {
      throw new NoSuchElementException("Start or End vertice cannot be located within the graph");
    }

    Path shortestPath = new Path(vertices.get(start));
    Path pathing;
    foundPaths.add(shortestPath);
    visited.add(vertices.get(start));
    unvisitedNodes.remove(vertices.get(start));

    while (!unvisitedNodes.isEmpty()) { // traverse until path is found or all nodes are visited
      shortestPath = foundPaths.remove();
      // the first path will be the shortest out of the entire priorityQueue

      if (end.equals(shortestPath.end.data)) {
        return shortestPath; // if the shortest path is found, return the path
      } else {
        unvisitedNodes.remove(shortestPath.end);
        visited.add(shortestPath.end);

        for (int i = 0; i < shortestPath.end.edgesLeaving.size(); i++) {
          if (!visited.contains(shortestPath.end.edgesLeaving.get(i).target)) {
            pathing = new Path(shortestPath, shortestPath.end.edgesLeaving.get(i));
            foundPaths.add(pathing);
          }
        }
      }
    }
    throw new NoSuchElementException("No paths to the end vertice were found within the graph");
  }
