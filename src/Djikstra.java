...
public class Djikstra<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public LinkedList<T> dataSequence; // ordered sequence of data from vertices in path
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      this.dataSequence = (LinkedList<T>) copyPath.dataSequence.clone();
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    Hashtable<T, Path> bestPath = new Hashtable<>();
    Path curr;
    int weight;
    ...
    dist.put(start, 0);
    Path tempPath = new Path(vertices.get(start));
    frontier.add(tempPath);

    while (!frontier.isEmpty()) {
      curr = frontier.poll();
      for (Edge e : curr.end.edgesLeaving) {
        weight = e.weight + curr.distance;
        if (dist.get(e.target.data) > weight) {
          dist.put(e.target.data, weight);
          bestPath.put(e.target.data, new Path(curr, e));
          frontier.add(new Path(curr, e));
        }
      }
    }
    if (bestPath.get(end) == null) {
      throw new NoSuchElementException();
    }
    return bestPath.get(end);
